...
def handpercsq(hand, board, eff = 0):
    ...
    startinghands['rank'] = [[h[0:2],h[2:4]] for h in startinghands[0]]
    if eff == 0:
        startinghands['rank'] = [calculate.handpercentile(h,board) for h in startinghands['rank']]
    elif eff == 1:
        startinghands['rank'] = [calculate.effectivepercentile(h,board) for h in startinghands['rank']]
    startinghands = startinghands.set_index(0)
